...
Пропуски в тренировочном CSV **не заполняются при загрузке**: они остаются как `NaN` и обрабатываются `SimpleImputer` внутри ML pipeline / после `train_test_split` (fit только на train). Строки без `churn` удаляются.
Пример ответа:
